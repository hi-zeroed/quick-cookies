...
<p align="center">
  <strong>极速无感、卡片式 macOS 开源文件快速预览工具</strong>
</p>
...
**Quick Cookies** 是一款专为 macOS 设计的极轻量、卡片式文件快速预览工具。它致力于消除传统“双击打开文件”的繁琐步骤，通过全局双击 `⌘ Command` 热键实现文件的 0ms 瞬间预览，并支持一键在系统默认应用或外部专业编辑器（VS Code, Cursor, Xcode 等）中打开，无缝衔接后续工作流。

界面采用高对比度的毛玻璃 HUD 视感与黄金阅读比例，配备原生的中心弹性微缩弹簧动效（Center Spring），与 macOS 访达（Finder）深度集成且完全免除辅助功能权限依赖，为您的日常研发、文档审阅与阅读带来无感、丝滑的极速体验。
...
- 🚀 **0ms 闪电级响应弹出**
  在按下快捷键的瞬间，窗口立即以原生中心物理弹簧 0ms 膨胀起跳，文件内容于后台 Task 异步解密、加载与语法高亮渲染。体验行云流水，绝不抢占或挂起主线程，彻底消除大文件读取造成的视觉卡顿。
- 📐 **黄金书籍阅读比例**
...
- ⚡️ **外部专业编辑器一键直开 (Open in External Editor)**
  告别简陋且缺乏 LSP 上下文的内嵌轻量编辑，聚焦极致预览体验。顶栏右侧毛玻璃胶囊一键以系统默认应用打开；下拉菜单智能感知已安装的专业编辑器（VS Code, Cursor, Xcode, Typora, Zed 等），并支持 `⌘O` / `⌘Return` 快捷打开、`⌘R` 在访达中显示、`⌥⌘C` 复制文件路径。
- 📊 **Office 文档与媒体格式支持**
...
- 🔌 **系统级原生深度融合**
  ...
  - **Scripting Bridge 高性能检测**：通过内存级 AppleEvent 零开销同步获取 Finder 当前高亮文件路径。
  - **原生中心微缩动效与 0 辅助功能依赖**：采用 macOS 原生中心物理弹簧展开与流体三次贝塞尔收缩，彻底解绑系统高危辅助功能权限，即装即用。
...
├── Core/          # 核心底层引擎 (热键路由, 外部编辑器直开 ExternalAppRelay, 物理窗口管理)
...
| **用默认应用打开**        | <kbd>⌘ Command</kbd> + <kbd>O</kbd> / <kbd>↩ Return</kbd> | 立即使用系统默认应用或首选编辑器打开当前文件，并自动关闭预览浮层。                                                         |
