...
- **UI 细节优化与动效打磨（2026-06-01）**：
  - **PDF与Office视图物理圆角裁剪**：除在 SwiftUI 应用 `.cornerRadius(12)` 外，我们直接在 [MediaPreviewView.swift](QuickCookies/UI/MediaPreviewView.swift) 中的 `PDFKitView` 和 [OfficePreviewView.swift](QuickCookies/UI/OfficePreviewView.swift) 中的 `QLPreviewView` 两个 AppKit 桥接实现中，显式配置了底层 `wantsLayer = true` 以及 `layer.cornerRadius = 12` 与 `layer.masksToBounds = true`，从物理层级上完成了彻底的圆角剪切，彻底解决渲染底色与背景分层导致的直角溢出 Bug。
  - **Markdown 导出交互提升 (线性进度条与居上无 Icon 提醒)**：在 [ContentView.swift](QuickCookies/UI/ContentView.swift) 中，当用户开始导出 PDF 时，工具栏右侧的“导出 PDF”按钮会原位无痕转化为横向无限滑动的线性进度条，移除全屏 Loading 骨架屏；PDF 导出成功或失败后，会在窗口顶部居中位置（`.top` 且 `padding(.top, 60)`）优雅地由顶向下滑入淡出半透明高置信度提示框，且完全去除了 ℹ️ 和 ✗ 等冗余 Icon 字符。
