...
class AppDelegate: NSObject, NSApplicationDelegate {
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // 注册 Services 菜单项
        NSApp.servicesProvider = QuickPeekServiceProvider()

        print("QuickPeek started. Double-press Option key to preview files.")
    }
...
class QuickPeekServiceProvider: NSObject {
    /// Services 菜单项：打开 QuickPeek
    @objc func quickPeekService(_ pasteboard: NSPasteboard, userData: String?, error: AutoreleasingUnsafeMutablePointer<NSString>) {
        // 1. 尝试作为 URL 读取
